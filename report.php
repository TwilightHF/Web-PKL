...
<head>
    ...
    <style>
        ...
        .report-summary-card h6 { color: #6c757d; font-size: .82rem; margin-bottom: 4px; }
        .report-summary-card h3 { font-weight: 700; margin-bottom: 4px; }
        .report-summary-card .trend { font-size: .78rem; }
        ...
        .status-legend-row { display: flex; justify-content: space-between; align-items: center; padding: 6px 0; font-size: .85rem; }
        .top-customer-table td, .top-customer-table th { font-size: .88rem; }
        #periodPicker { cursor: pointer; background: white; }
        .table-sm-badge { font-size: .72rem; padding: 5px 10px; }
    </style>
...
<body>
    ...
    // ============================================================
    // Sumber data: api/report_data.php (proxy PHP ke Apps Script mode
    // ?report=1), yang mengembalikan field sesuai buildReportTasks():
    //   id, tipe, customer, area, district, sow, milestone, status,
    //   responsibility, priorityDetail, dibuat, oa_date, ttd, catatan
    // ============================================================
    const API_URL = "api/report_data.php";

    let allTasksRaw = [];
    let allOnAirRaw = [];  // data khusus On Air Trend (AZ = OA, tanpa filter role)
    let filteredTasks = [];
    let currentPage = 1;
    ...
    async function fetchData() {
        try {
            ...
            allOnAirRaw = Array.isArray(data.onAirTrend) ? data.onAirTrend : [];
    ...
    // ------------------------------------------------------------
    // Mapping field (mengikuti buildReportTasks di code.gs):
    //   filterArea      -> t.area       (Region / Z)
    //   filterSla       -> t.sow        (Sow Order / I)
    //   filterPrioritas -> t.milestone  (Milestone / O)
    // ------------------------------------------------------------
    function populateFilterOptions() {
        fillSelect("filterArea", uniqueValues("area"), "Semua");
        fillSelect("filterSla", uniqueValues("sow"), "Semua");
        fillSelect("filterPrioritas", uniqueValues("milestone"), "Semua");
    ...
    function applyFilters() {
        ...
        const sow = document.getElementById("filterSla").value.toLowerCase();
        const milestone = document.getElementById("filterPrioritas").value.toLowerCase();
        ...
        filteredTasks = allTasksRaw.filter(t => {
            ...
            if (search) {
                const hay = [t.id, t.tipe, t.customer, t.area, t.milestone, t.status]
    ...
    function renderSummaryCards() {
        ...
        filteredTasks.forEach(t => {
            ...
            if (!isNaN(ttd)) {
                ttdSum += ttd; ttdCount++;
                if (ttd <= 20) onTrack++; // konsisten dgn ambang "Priority Order > 20 Hari"
            }
        });
    ...
    function renderResponsibilityDoughnut() {
        ...
        const colorMap = { "SO": "#ef4444", "MBB/SO": "#3b82f6", "DWS": "#f59e0b", "TA/ED": "#22c55e", "OA": "#8b5cf6", "Tidak diketahui": "#6c757d" };
        const palette = ["#0dcaf0", "#fd7e14", "#20c997", "#d63384"];
    ...
    // ============================================================
    // CHART: On Air Trend
    // Sumber: allOnAirRaw (khusus AZ="OA", TANPA filter role - terpisah
    // dari filteredTasks). Tetap ikut filter rentang tanggal saja.
    // ============================================================
    ...
    // Sumber: t.milestone (kolom O / Milestone) - label sesuai isi
    // asli spreadsheet, mis. "05. L0 - Perizinan", "0. Issue - Site BTS"
    // ============================================================
    function renderPriorityChart() {
        const counts = {};
        filteredTasks.forEach(t => {
            const p = (t.milestone || "Tidak diketahui").toString().trim() || "Tidak diketahui";
        ...
        const colors = labels.map(l => {
            ...
            if (low.includes("issue")) return "#ef4444";
            if (low.includes("ready")) return "#22c55e";
            return "#3b82f6";
        });
    ...
    // Berbasis t.ttd (kolom BI): dianggap "on track" kalau <= 20 hari.
    // Grouping TETAP bulanan - TIDAK mengikuti dropdown Harian/Mingguan/
    // Bulanan milik "On Air Trend" (dua chart ini independen).
    // ============================================================
    ...
    // Sumber: t.district (kolom WITEL / Y)
    // ============================================================
    ...
    function slaBadge(ttdVal) {
        ...
        if (ttd <= 20) return `<span class="badge bg-success table-sm-badge">On Track</span>`;
        return `<span class="badge bg-danger table-sm-badge">Over SLA</span>`;
    }
    ...
    function openDetailModal(id) {
        ...
        document.getElementById("mdSisaWaktu").textContent = (t.ttd !== undefined && t.ttd !== null && t.ttd !== "") ? t.ttd + " Hari" : "-";
    ...
    function exportCsv() {
        ...
        const headers = ["ID Task", "Tipe", "Customer", "Area", "Milestone", "SoW", "Status", "Waktu Dibuat"];
        const rows = filteredTasks.map(t => [
            t.id, t.tipe, t.customer, t.area, t.milestone, t.sow, normalizeStatus(t.status), t.dibuat
        ]);
